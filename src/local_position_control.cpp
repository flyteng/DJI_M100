...
{
  ros::init(argc, argv, "local_pos_control_node");
  ros::NodeHandle nh;
...
void local_position_ctrl(double &xCmd, double &yCmd, double &zCmd)
{
  ...
  if (((std::abs(xCmd)) < 0.1) && ((std::abs(yCmd)) < 0.1) && (local_position.point.z > (target_offset_z - 0.1)) && (local_position.point.z < (target_offset_z + 0.1))) {
